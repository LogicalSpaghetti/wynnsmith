import {damageTypePrefixes} from "../../common/small_stuff.ts";
...
export const skillPointNames = ["strength", "dexterity", "intelligence", "defence", "agility"];
const capitalizedSkillPointNames = ["Strength", "Dexterity", "Intelligence", "Defence", "Agility"];

export const SkillPointIndexes = Object.freeze({
    Strength: 0,
    Dexterity: 1,
    Intelligence: 2,
    Defence: 3,
...
export function capSkillPoint(sp) {
    return isNaN(sp) ? 0 : Math.min(Math.max(sp, 0), sp_cap);
}

// TODO: account for the 100 assign limit
export function getSPBalanceModifiers(strength, dexterity, spare) {
    const difference = Math.max(0, Math.abs(strength - dexterity));
    const overBalance = spare - difference;

    if (strength > sp_cap && dexterity > sp_cap) return {strength: 0, dexterity: 0};
    if (strength > sp_cap) return {
        strength: 0,
        dexterity: Math.max(0, Math.min(sp_cap, spare + dexterity) - dexterity)
    };
    if (dexterity > sp_cap) return {strength: Math.max(0, Math.min(sp_cap, spare + strength) - strength), dexterity: 0};
...
export function getItemAddedSP(item) {
    return capitalizedSkillPointNames.map((name) => Number(item?.identifications[`raw${name}`] ?? 0));
}

export function getSkillPointName(index, capitalize = true) {
    return (capitalize ? capitalizedSkillPointNames : skillPointNames)[index];
}

// TODO: loop through all tomes, equipment, and the weapon, and add their getItemAddedSP to the build.skill_points
export function calculateIdSkillPoints(build) {
    getItemAddedSP();
}

export function balanceSP() {
    ...
    // TODO: use assigned to cap below 100;
