const aspects = {
    "archer": [
        {
            "name": "Aspect of the Beastmaster",
        }, {
            "name": "Aspect of Battlement Fortification",
        }, {
            "name": "Aspect of the Inexhaustible Quiver",
        }, {
            "name": "Aspect of the North Wind",
        }, {
            "name": "Aspect of Undercrank",
        }, {
            "name": "Aspect of Clinging Lichen",
        }, {
            "name": "Aspect of Bullet Hell",
        }, {
            "name": "Aspect of Fragmentation Rounds",
        }, {
            "name": "Trapper's Embodiment of Persistence Predation",
        }, {
            "name": "Aspect of Further Horizons",
        }, {
            "name": "Aspect of Extreme Firepower",
        }, {
            "name": "Sharpshooter's Embodiment of Laser Precision",
        }, {
            "name": "Aspect of the Poltergeist",
        }, {
            "name": "Aspect of the Great Escape",
        }, {
            "name": "Aspect of the Steadying Hand",
        }, {
            "name": "Aspect of a Thunderbolt",
        }, {
            "name": "Aspect of Chaotic Demolition",
        }, {
            "name": "Aspect of Illegal Explosives",
        }, {
            "name": "Boltslinger's Embodiment of Rended Skies",
        }, {
            "name": "Aspect of Extreme Current",
        }, {
            "name": "Aspect of Dynamic Entry",
        }, {
            "name": "Aspect of the Iron String",
        }],
    "assassin": [
        {
            "name": "Aspect of the Disappearing Act",
        }, {
            "name": "Aspect of False Coercing",
        }, {
            "name": "Aspect of Visual Distortion",
        }, {
            "name": "Aspect of Redoublement",
        }, {
            "name": "Aspect of Flamboyance",
        }, {
            "name": "Aspect of Shadow Armor",
        }, {
            "name": "Aspect of the Fog Machine",
        }, {
            "name": "Aspect of Enduring Illusions",
        }, {
            "name": "Aspect of the Chain Knife",
        }, {
            "name": "Aspect of the Calling Card",
        }, {
            "name": "Aspect of the Unstoppable Force",
        }, {
            "name": "Aspect of the Dagger's Silhouette",
        }, {
            "name": "Aspect of the Agile Blade",
        }, {
            "name": "Aspect of the Stellar Flurry",
        }, {
            "name": "Aspect of the Pernicious Prankster",
        }, {
            "name": "Aspect of Sleight-Of-Hand",
        }, {
            "name": "Trickster's Embodiment of Malevolent Mischief",
        }, {
            "name": "Acrobat's Embodiment of Gravity Defiance",
        }, {
            "name": "Aspect of Seeking Stars",
        }, {
            "name": "Aspect of Unyielding Fate",
        }, {
            "name": "Aspect of Athlethicism",
        }, {
            "name": "Shadestepper's Embodiment of Unseen Execution",
        }, {
            "name": "Aspect of the Pinwheel",
        }, {
            "name": "Aspect of Clouded Vision",
        }, {
            "name": "Aspect of the Airborne",
        }],
    "mage": [
        {
            "name": "Aspect of Mystic Transfer",
        }, {
            "name": "Aspect of the Savior",
        }, {
            "name": "Aspect of the Comet",
        }, {
            "name": "Light Bender's Embodiment of Celestial Brilliance",
        }, {
            "name": "Aspect of Runic Extravagance",
        }, {
            "name": "Aspect of a Scorching Sun",
        }, {
            "name": "Aspect of Manaflux",
        }, {
            "name": "Aspect of a Thousand Hours",
        }, {
            "name": "Aspect of the Apprentice's Bolt",
        }, {
            "name": "Aspect of Shining Status",
        }, {
            "name": "Aspect of Wind Walking",
        }, {
            "name": "Aspect of the Dimension's Door",
        }, {
            "name": "Aspect of the Ray of Frost",
        }, {
            "name": "Aspect of the Magic Missile",
        }, {
            "name": "Riftwalker's Embodiment of Chronal Control",
        }, {
            "name": "Arcanist's Embodiment of Total Obliteration",
        }, {
            "name": "Aspect of Burning Providence",
        }, {
            "name": "Aspect of Flickering Transmission",
        }, {
            "name": "Aspect of Fatal Fulguration",
        }],
    "shaman": [
        {
            "name": "Aspect of the Amphibian",
            "rarity": "fabled",
            "descriptions": [
                "§7§nFrog Dance§7 gains §f+2§7 bounces.",
                "§7§nFrog Dance§7 gains §f+3§7 bounces.",
                "§7§nFrog Dance§7 gains §f+4§7 bounces.",
            ],
        }, {
            "name": "Aspect of Motivation",
            "rarity": "legendary",
            "descriptions": [
                "§7§nBullwhip§7 gains §f+5%§7 Damage Bonus to §bSummons§7.",
                "§7§nBullwhip§7 gains §f+9%§7 Damage Bonus to §bSummons§7.",
                "§7§nBullwhip§7 gains §f+12%§7 Damage Bonus to §bSummons§7.",
                "§7§nBullwhip§7 gains §f+15%§7 Damage Bonus to §bSummons§7.",
            ]
        }, {
            "name": "Summoner's Embodiment of the Omnipotent Overseer",
            "rarity": "mythic",
            "descriptions": [
                "§7§nTriple Totem§7 gains §f+1§7 §nTotem§7.\n§nTotem§7 and §nAura§7 will deal §c55%§7 less damage.\n\n§nRegeneration§7 and §nSacrificial Shrine§7\nwill heal §c55%§7 less health.",
                "§7§nTriple Totem§7 gains §f+1§7 §nTotem§7.\n§nTotem§7 and §nAura§7 will deal §c55%§7 less damage.\n\n§nRegeneration§7 and §nSacrificial Shrine§7\nwill heal §c55%§7 less health.\n\n§nHummingbird's Song§7 gains §f+1 §bHummingbird§7.",
                "§7§nTriple Totem§7 gains §f+1§7 §nTotem§7.\n§nTotem§7 and §nAura§7 will deal §c55%§7 less damage.\n\n§nRegeneration§7 and §nSacrificial Shrine§7\nwill heal §c55%§7 less health.\n\n§nHummingbird's Song§7 gains §f+1 §bHummingbird§7.\n§bHummingbirds§7 gain §f+8§7 §a➼ §7Range.",
            ],
        }, {
            "name": "Aspect of Occupation",
            "rarity": "legendary",
            "descriptions": [
                "Your §bHummingbirds§7 attack enemies for §f+3.5s§7.",
                "Your §bHummingbirds§7 attack enemies for §f+6s§7.",
                "Your §bHummingbirds§7 attack enemies for §f+8.5s§7.",
                "Your §bHummingbirds§7 attack enemies for §f+10s§7.",
            ]
        }, {
            "name": "Aspect of Incineration",
            "rarity": "legendary",
            "descriptions": [
                "§7§nFriendly Fire§7 reduces§b Puppet§7 duration by §f-0.4s§7.",
                "§7§nFriendly Fire§7 reduces§b Puppet§7 duration by §f-0.6s§7.",
                "§7§nFriendly Fire§7 reduces§b Puppet§7 duration by §f-0.8s§7.",
                "§7§nFriendly Fire§7 reduces§b Puppet§7 duration by §f-1s§7.",
            ]
        }, {
            "name": "Acolyte's Embodiment of Unwavering Adherence",
            "rarity": "mythic",
            "descriptions": [
                "§7§nBlood Sorrow§7 gains §f+1s§7 §d⌛§7 Duration.",
                "§7§nBlood Sorrow§7 gains §f+1s§7 §d⌛§7 Duration.\n\n§7§nEldritch Call§7 summons §f+1§7 tentacle, and strikes §f20%§7 faster.",
                "§7§nBlood Sorrow§7 gains §f+2s§7 §d⌛§7 Duration.\n\n§7§nEldritch Call§7 summons §f+1§7 tentacle, and strikes §f20%§7 faster.",
            ],
        }, {
            "name": "Aspect of Stances",
            "rarity": "fabled",
            "descriptions": [
                "§7§nMask of the Lunatic§7 gains §f+5% §e✧ §7Damage Bonus.\n\n§7§nMask of the Fanatic§7 gains §f+5% §e✧ §7Resistance.\n\n§7§nMask of the Heretic§7 gains §f+10% §e✧ §7Walk Speed.",
                "§7§nMask of the Lunatic§7 gains §f+8% §e✧ §7Damage Bonus.\n\n§7§nMask of the Fanatic§7 gains §f+8% §e✧ §7Resistance.\n\n§7§nMask of the Heretic§7 gains §f+16% §e✧ §7Walk Speed.",
                "§7§nMask of the Lunatic§7 gains §f+10% §e✧ §7Damage Bonus.\n\n§7§nMask of the Fanatic§7 gains §f+10% §e✧ §7Resistance.\n\n§7§nMask of the Heretic§7 gains §f+20% §e✧ §7Walk Speed.",
            ],
        }, {
            "name": "Aspect of Lashing Fire",
            "rarity": "legendary",
            "descriptions": [
                "§7§nFlaming Tongue§7 gains §f+1§7 hit, and doubles in speed.\n\n§c⚔ §7Total Damage: §c-14% §8(of your DPS)\n   §8(§6✣ §8Damage: -11%)\n   §8(§e✦ §8Thunder: -1%)\n   §8(§c✹ §8Fire: -2%)",
                "§7§nFlaming Tongue§7 gains §f+2§7 hits, and doubles in speed.\n\n§c⚔ §7Total Damage: §c-23% §8(of your DPS)\n   §8(§6✣ §8Damage: -17%)\n   §8(§e✦ §8Thunder: -2%)\n   §8(§c✹ §8Fire: -4%)",
                "§7§nFlaming Tongue§7 gains §f+3§7 hits, and doubles in speed.\n\n§c⚔ §7Total Damage: §c-29% §8(of your DPS)\n   §8(§6✣ §8Damage: -20%)\n   §8(§e✦ §8Thunder: -3%)\n   §8(§c✹ §8Fire: -6%)",
                "§7§nFlaming Tongue§7 gains §f+4§7 hits, and doubles in speed.\n\n§c⚔ §7Total Damage: §c-34% §8(of your DPS)\n   §8(§6✣ §8Damage: -22%)\n   §8(§e✦ §8Thunder: -4%)\n   §8(§c✹ §8Fire: -8%)",
            ]
        }, {
            "name": "Aspect of Reverberation",
            "rarity": "legendary",
            "descriptions": [
                "§7§nTotemic Smash§7 gains §f+1.25 §3☀ §7Area of Effect.",
                "§7§nTotemic Smash§7 gains §f+2.25 §3☀ §7Area of Effect.",
                "§7§nTotemic Smash§7 gains §f+3 §3☀ §7Area of Effect.",
                "§7§nTotemic Smash§7 gains §f+4 §3☀ §7Area of Effect.",
            ]
        }, {
            "name": "Aspect of Exsanguination",
            "rarity": "fabled",
            "descriptions": [
                "§7Increase your maximum §bBlood Pool\n§4⚕ §7and §bBleeding §c §7duration by §f+17%§7.",
                "§7Increase your maximum §bBlood Pool\n§4⚕ §7and §bBleeding §c §7duration by §f+25%§7.",
                "§7Increase your maximum §bBlood Pool\n§4⚕ §7and §bBleeding §c §7duration by §f+33%§7.",
            ],
        }, {
            "name": "Aspect of Acceleration",
            "rarity": "legendary",
            "descriptions": [
                "Your §bPuppets§7 and §bEffigies§7 move §f10%§7 faster.",
                "Your §bPuppets§7 and §bEffigies§7 move §f15%§7 faster.",
                "Your §bPuppets§7 and §bEffigies§7 move §f20%§7 faster.",
                "Your §bPuppets§7 and §bEffigies§7 move §f25%§7 faster.",
            ]
        }, {
            "name": "Aspect of the Channeler",
            "rarity": "fabled",
            "descriptions": [
                "§7§nChant of the Lunatic§7 gains §f+1.25s §d⌛ §7Duration.\n\n§7§nChant of the Fanatic§7 gains §f+0.35s §d⌛ §7Duration.\n\n§7§nChant of the Heretic§7 takes §f+15% §7longer to decay.",
                "§7§nChant of the Lunatic§7 gains §f+2.25s §d⌛ §7Duration.\n\n§7§nChant of the Fanatic§7 gains §f+0.7s §d⌛ §7Duration.\n\n§7§nChant of the Heretic§7 takes §f+25% §7longer to decay.",
                "§7§nChant of the Lunatic§7 gains §f+3s §d⌛ §7Duration.\n\n§7§nChant of the Fanatic§7 gains §f+1s §d⌛ §7Duration.\n\n§7§nChant of the Heretic§7 takes §f+35% §7longer to decay.",
            ],
        }, {
            "name": "Aspect of the Alraune's Roots",
            "rarity": "legendary",
            "descriptions": [
                "§7§nUproot§7 and §nHaunting Memory§7\ngain §f+2§7 §a➼§7 Range.",
                "§7§nUproot§7 and §nHaunting Memory§7\ngain §f+3.5§7 §a➼§7 Range.",
                "§7§nUproot§7 and §nHaunting Memory§7\ngain §f+5§7 §a➼§7 Range.",
                "§7§nUproot§7 and §nHaunting Memory§7\ngain §f+6§7 §a➼§7 Range.",
            ],
        }, {
            "name": "Aspect of Seismic Sense",
            "rarity": "fabled",
            "descriptions": [
                "§7§nNature's Jolt§7 gains §f+1.25 §3☀ §7Area of Effect.",
                "§7§nNature's Jolt§7 gains §f+2.25 §3☀ §7Area of Effect.",
                "§7§nNature's Jolt§7 gains §f+3 §3☀ §7Area of Effect.",
            ],
        }, {
            "name": "Aspect of the Monolith",
            "rarity": "legendary",
            "descriptions": [
                "§7§nTotem§7 gains §f10s §d⌛§7 Duration.",
                "§7§nTotem§7 gains §f18s §d⌛§7 Duration.",
                "§7§nTotem§7 gains §f24s §d⌛§7 Duration.",
                "§7§nTotem§7 gains §f30s §d⌛§7 Duration.",
            ],
        }, {
            "name": "Aspect of Emanant Force",
            "rarity": "legendary",
            "descriptions": [
                "§7§nTotem§7 gains §f+1§7 §3☀§7 Area of Effect.\n\n§7§nEldritch Call§7 gains §f+2§7 §3☀§7 Area of Effect.",
                "§7§nTotem§7 gains §f+1.75§7 §3☀§7 Area of Effect.\n\n§7§nEldritch Call§7 gains §f+3.5§7 §3☀§7 Area of Effect.",
                "§7§nTotem§7 gains §f+2.5§7 §3☀§7 Area of Effect.\n\n§7§nEldritch Call§7 gains §f+5§7 §3☀§7 Area of Effect.",
                "§7§nTotem§7 gains §f+3§7 §3☀§7 Area of Effect.\n\n§7§nEldritch Call§7 gains §f+6§7 §3☀§7 Area of Effect.",
            ],
        }, {
            "name": "Aspect of Surging Presence",
            "rarity": "legendary",
            "descriptions": [
                "§7§nAura§7 travels §f+0.1s§7 faster.",
                "§7§nAura§7 travels §f+0.2s§7 faster.",
                "§7§nAura§7 travels §f+0.25s§7 faster.",
                "§7§nAura§7 travels §f+0.3s§7 faster.",
            ],
        }, {
            "name": "Aspect of Empathy",
            "rarity": "legendary",
            "descriptions": [
                "§7§nTwisted Tether§7 requires §f-0.2%§7 Health loss to activate.",
                "§7§nTwisted Tether§7 requires §f-0.3%§7 Health loss to activate.",
                "§7§nTwisted Tether§7 requires §f-0.4%§7 Health loss to activate.",
                "§7§nTwisted Tether§7 requires §f-0.5%§7 Health loss to activate.",
            ]
        }, {
            "name": "Aspect of the Beckoned Legion",
            "rarity": "fabled",
            "descriptions": [
                "§7§nPuppet Master§7 gains §f+1 §bPuppets§7.\n\n§c⚔ §7Total Damage: §c-1% §8(of your DPS)\n   §8(§2✤ §8Earth: -1%)",
                "§7§nPuppet Master§7 gains §f+2 §bPuppets§7.\n\n§c⚔ §7Total Damage: §c-2% §8(of your DPS)\n   §8(§f❋ §8Air: -1%)\n   §8(§2✤ §8Earth: -1%)",
                "§7§nPuppet Master§7 gains §f+3 §bPuppets§7.\n\n§c⚔ §7Total Damage: §c-3% §8(of your DPS)\n   §8(§6✣ §8Damage: -1%)\n   §8(§f❋ §8Air: -1%)\n   §8(§2✤ §8Earth: -1%)",
            ],
        }, {
            "name": "Ritualist's Embodiment of the Ancestral Avatar",
            "rarity": "mythic",
            "descriptions": [
                "§7§nAwakened§7 requires §f-30§7 mana to activate.",
                "§7§nAwakened§7 requires §f-30§7 mana to activate.\n§7§nAwakened§7 gains §f+10s§7 §d⌛§7 Duration.",
                "§7§nAwakened§7 requires §f-50§7 mana to activate.\n§7§nAwakened§7 gains §f+15s§7 §d⌛§7 Duration.",
            ],
        }, {
            "name": "Aspect of Summer Storms",
            "rarity": "legendary",
            "descriptions": [
                "§7Increase §nStorm Dance§7's pull strength by §f10%§7.",
                "§7Increase §nStorm Dance§7's pull strength by §f10%§7.",
                "§7Increase §nStorm Dance§7's pull strength by §f10%§7.",
                "§7Increase §nStorm Dance§7's pull strength by §f25%§7.",
            ],
        }],
    "warrior": [
        {
            "name": "Aspect of Steel Chords",
        }, {
            "name": "Aspect of Empowering Fantasy",
        }, {
            "name": "Aspect of Earthshaking",
        }, {
            "name": "Aspect of Bovine Inspiration",
        }, {
            "name": "Battle Monk's Embodiment of Complete Synchrony",
        }, {
            "name": "Fallen's Embodiment of Blind Fury",
        }, {
            "name": "Aspect of Seeing Stars",
        }, {
            "name": "Aspect of Maniacal Frisson",
        }, {
            "name": "Aspect of Overflowing Hope",
        }, {
            "name": "Aspect of Unquenching Flames",
        }, {
            "name": "Aspect of the Tightrope Walk",
        }, {
            "name": "Aspect of Rekindling",
        }, {
            "name": "Aspect of Rallying Fervor",
        }, {
            "name": "Aspect of the Anvil Drop",
        }, {
            "name": "Aspect of Deafening Echoes",
        }, {
            "name": "Aspect of the Berserker",
        }, {
            "name": "Aspect of Turbulence",
        }, {
            "name": "Paladin's Embodiment of Undying Determination",
        }, {
            "name": "Aspect of Hyper-Perception",
        }, {
            "name": "Aspect of Skyward Strikes",
        }, {
            "name": "Aspect of the Returning Javelin",
        }, {
            "name": "Aspect of the Megaphone",
        }],
};