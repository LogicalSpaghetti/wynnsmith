import {damageTypePrefixes} from "../../to_sort/small_stuff.ts";
import type {NormalItemData} from "../item_types.ts";

export const sp_indexes = 5;
const sp_cap = 150;

const sp_multipliers = [
    0,
    0.00989964625160766,
    0.0197155089482952,
    0.0294410876039912,
    0.0390772126304875,
    0.0486247068016281,
    0.058084385323578,
    0.0674570559044188,
    0.0767435188231318,
    0.0859445669979048,
    0.0950609860538511,
    0.104093554390086,
    0.113043043246184,
    0.121910216768048,
    0.130695832073133,
    0.139400639315104,
    0.148025381747884,
    0.156570795789126,
    0.165037611083075,
    0.17342655056288,
    0.181738330512316,
    0.189973660626943,
    0.198133244074714,
    0.206217777555997,
    0.214227951363072,
    0.222164449439068,
    0.230027949436372,
    0.237819122774471,
    0.245538634697294,
    0.253187144330006,
    0.2607653047353,
    0.268273762969142,
    0.275713160136026,
    0.283084131443717,
    0.290387306257482,
    0.297623308153841,
    0.30479275497379,
    0.311896258875572,
    0.318934426386934,
    0.325907858456931,
    0.332817150507218,
    0.339662892482903,
    0.346445668902912,
    0.353166058909902,
    0.359824636319714,
    0.366421969670357,
    0.372958622270561,
    0.37943515224787,
    0.385852112596306,
    0.392210051223573,
    0.398509510997849,
    0.404751029794134,
    0.410935140540178,
    0.417062371261991,
    0.423133245128911,
    0.429148280498293,
    0.435107990959753,
    0.44101288537904,
    0.446863467941463,
    0.452660238194953,
    0.458403691092717,
    0.464094317035492,
    0.469732601913429,
    0.475319027147568,
    0.480854069730951,
    0.486338202269345,
    0.491771893021606,
    0.497155605939646,
    0.502489800708057,
    0.507774932783356,
    0.513011453432876,
    0.518199809773304,
    0.523340444808842,
    0.528433797469046,
    0.533480302646296,
    0.53848039123294,
    0.543434490158068,
    0.548343022423979,
    0.553206407142296,
    0.558025059569744,
    0.562799391143622,
    0.567529809516916,
    0.572216718593116,
    0.576860518560698,
    0.581461605927306,
    0.586020373553587,
    0.590537210686753,
    0.595012502993808,
    0.599446632594477,
    0.603839978093844,
    0.608192914614663,
    0.612505813829398,
    0.616779043991954,
    0.621012969969127,
    0.625207953271745,
    0.629364352085548,
    0.633482521301762,
    0.63756281254741,
    0.641605574215329,
    0.645611151493916,
    0.649579886396609,
    0.653512117791082,
    0.657408181428186,
    0.661268409970609,
    0.665093133021288,
    0.668882677151544,
    0.672637365928975,
    0.676357519945076,
    0.680043456842616,
    0.68369549134276,
    0.687313935271936,
    0.690899097588472,
    0.694451284408963,
    0.697970799034416,
    0.701457941976142,
    0.704913010981426,
    0.708336301058934,
    0.711728104503914,
    0.71508871092315,
    0.718418407259688,
    0.721717477817341,
    0.724986204284958,
    0.72822486576048,
    0.731433738774769,
    0.734613097315221,
    0.73776321284916,
    0.740884354347013,
    0.743976788305282,
    0.747040778769292,
    0.750076587355747,
    0.753084473275053,
    0.756064693353462,
    0.759017502054998,
    0.761943151503184,
    0.764841891502566,
    0.767713969560049,
    0.770559630906023,
    0.773379118515307,
    0.776172673127896,
    0.778940533269511,
    0.78168293527197,
    0.784400113293363,
    0.787092299338052,
    0.789759723276473,
    0.792402612864768,
    0.795021193764229,
    0.797615689560572,
    0.800186321783018,
    0.802733309923217,
    0.805256871453982,
    0.807757221847863,
] as const;

export const skillPointNames = ["strength", "dexterity", "intelligence", "defence", "agility"] as const;
const capitalizedSkillPointNames = ["Strength", "Dexterity", "Intelligence", "Defence", "Agility"] as const;

export const SkillPointIndexes = {
    Strength: 0,
    Dexterity: 1,
    Intelligence: 2,
    Defence: 3,
    Agility: 4,
} as const;

export function getSkillPointMultiplier(points: number, index: number) {
    let mlt = sp_multipliers[capSkillPoint(points)];
    if (index === SkillPointIndexes.Defence) mlt *= 0.867;
    if (index === SkillPointIndexes.Agility) mlt *= 0.951;

    return mlt;
}

export function capSkillPoint(sp: number) {
    return isNaN(sp) ? 0 : Math.min(Math.max(sp, 0), sp_cap);
}

export function getItemAddedSP(item: NormalItemData | null) {
    if (!item || item.identifications) return [0, 0, 0, 0, 0];
    return capitalizedSkillPointNames.map((name) =>
        Number(item.identifications?.[`raw${name}`] ?? 0));
}

export function getItemSPReqs(item: NormalItemData | null) {
    const reqs = item?.requirements;
    return reqs ? skillPointNames.map(name => Number(reqs[name] ?? 0)) : [0, 0, 0, 0, 0];
}

export function getSkillPointName(index: number, capitalize = true) {
    return (capitalize ? capitalizedSkillPointNames : skillPointNames)[index];
}

export function getSkillPointModifiers() {
    const spClusters = document.getElementById("sp_section")?.querySelectorAll(".sp_cluster");
    if (!spClusters) return;

    const totals = [];
    for (let cluster of spClusters) {
        const modifierInput = cluster.querySelector(".sp_input") as HTMLInputElement;
        if (!modifierInput) return;
        const index = damageTypePrefixes.indexOf(((cluster as HTMLElement).dataset.element) as typeof damageTypePrefixes[number]) - 1;

        const value = Math.max(-1000, Math.min(1000, parseInt(modifierInput.value))) || 0;
        if (modifierInput.value === "0-" || modifierInput.value === "-0" || modifierInput.value === "-")
            modifierInput.value = "-";
        else modifierInput.value = String(value);

        totals[index] = value;
    }

    return totals;
}
